...
namespace Q_Tech_Bookkeeping
{
    public partial class Quotes : Form
    {
        ...
        private void Btn_LQ_Filter_Click(object sender, EventArgs e)
        {
            ...
            {
                dbConnection.Open();
                SqlDataAdapter sqlDataAdapter = new SqlDataAdapter("SELECT * FROM Quotes_Send WHERE Client = '" + CNAME + "' AND Date_Send BETWEEN '" + dtp_LQ_From.Value + "' AND '" + dtp_LQ_To.Value + "'", dbConnection);
                dt = new DataTable();
                sqlDataAdapter.Fill(dt);
            }
            bs.DataSource = dt;
